...
- It's deployed via `docker compose` (`docker-compose.yml` in that directory) as containers `dailyptr-web` (port 8000) and `dailyptr-postgres`. There's no sandbox/Docker-socket subsystem anymore (removed 2026-08-19, replaced with self-graded code review challenges) - `web` no longer needs `/var/run/docker.sock` mounted or a sandbox image built.
